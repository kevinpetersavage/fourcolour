...
class Configuration:
    ...
    @staticmethod
    def create_s_functions(partition):
    ...
    def create_signed_paths_for_non_extending(self):
        ...
        fitted = [signed_path for signed_path in signed_paths
                  if self.fits_at_least_one(signed_path)]
        return (signed_path for signed_path in fitted
                if not self.fits_at_least_one(signed_path))

    def fits_at_least_one(self, signed_path):
        return self.fits(signed_path)

    @staticmethod
    def fits(signed_path):
